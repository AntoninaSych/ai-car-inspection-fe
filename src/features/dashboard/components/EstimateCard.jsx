...
function getBadge(item, t) {
  if (item.status === ESTIMATE_STATUS.COMPLETED) {
    return {
      label: t('status.completed'),
      icon: <CheckCircleIcon fontSize="inherit" />,
      sx: { bgcolor: 'rgba(46, 204, 113, 0.15)', color: 'success.main' },
    };
  }
  if (item.isPaid) {
    return {
      ...
      sx: { bgcolor: 'rgba(255, 159, 67, 0.15)', color: 'warning.main' },
    };
  }
  return {
    label: t('status.paymentRequired'),
    icon: <PaymentsIcon fontSize="inherit" />,
    sx: { bgcolor: 'rgba(155, 89, 182, 0.15)', color: 'secondary.main' },
  };
}
...
export function EstimateCard({ item, onViewDetails, t }) {
  ...
  // TODO calculate estimated cost (take ones from a report
  const estimatedCost = item.isPaid ? '$850' : '';
  const estimatedCostPlaceholder = '******';
  const taskReports = item.reports ?? [];
  const lastReport = taskReports.at(-1);

  return (
    <Paper variant="outlined" sx={{ borderRadius: 3, overflow: 'hidden' }}>
      <Box sx={{ position: 'relative', height: 180, bgcolor: 'action.hover' }}>
        <Box
          component="img"
      ...
      <Box sx={{ p: 2 }}>
        ...
        <Stack direction="row" alignItems="flex-end" justifyContent="space-between" sx={{ mt: 2 }}>
          <Box>
            ...
            <Typography sx={{ fontSize: 20, fontWeight: 800 }}>{estimatedCost || estimatedCostPlaceholder}</Typography>
          </Box>
          {item.isPaid ? (
